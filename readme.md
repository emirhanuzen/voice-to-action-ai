...
Yapay Zeka (Dış API Bağımlılığı Yoktur): Whisper (Speech-to-Text), spaCy/NLTK (Özel Kural Tabanlı NLP), FFMPEG, gTTS

📅 11 Haftalık Geliştirme Planı ve İş Paketleri (Sprint Checklist)

Aşağıdaki liste, projenin Çevik (Agile) geliştirme sürecindeki haftalık hedeflerini göstermektedir. (Test süreçleri 3. haftadan itibaren her pakete entegre edilmiştir.)
...
Metni analiz edecek kural tabanlı (Rule-Based) NLP algoritmasının (spaCy/NLTK) yazılması.
...
Bu belgenin (README) kullanım rehberi içerecek şekilde güncellenmesi.
